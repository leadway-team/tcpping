...
void tcpsendw(string addr_s, ushort port, string msg) {
	auto tcps = new TcpSocket();
	auto addr = new InternetAddress(std.socket.InternetAddress.parse(addr_s), port);
	tcps.setOption(SocketOptionLevel.SOCKET, SocketOption.RCVTIMEO, 5.seconds);
	tcps.connect(addr);
	tcps.send(msg);
...
int quick(string[] args) {
	cwritefln("<b>Quick TcpPing</b>:<b><grey> v0.1.5</grey></b>");
	...
	tcpsend(tmp[0], to!ushort(tmp[1]), args[5]);
	
	return(0);
}

void shell() {
	cwritefln("<b>TcpPing Console</b>:<b><grey> v0.1.5 BETA</grey></b>");
	bool gogo = true;
	string addr_s = null;
	ushort port = 0;
